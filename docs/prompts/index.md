...
```bash
curl -X 'POST' \
  'http://localhost:8000/api/prompts/search' \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X 'POST' \
  'http://localhost:8000/api/prompts/{prompt_id}/v' \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
  -d '{
  "name": "Concise Summariser",
...
```bash
curl -X 'DELETE' \
  'http://localhost:8000/api/prompts/{prompt_id}/v/{v}' \
  -H 'Authorization: Bearer <token>'
```
...
```bash
curl -X 'PUT' \
  'http://localhost:8000/api/prompts/{prompt_id}/public' \
  -H 'Authorization: Bearer <token>'
```
...
```bash
curl 'http://localhost:8000/api/prompts/{prompt_id}/raw'
```
